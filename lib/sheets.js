...
async function cleanupClaimedRows(sheets, sheetTab, sheetId) {
  try {
    ...
    const orderCodes = new Set((ordersRes.data.values || []).map(r => (r[0] || '').trim().toUpperCase()));
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (isTimestampMarker) {
        ...
        // Order already saved → delete the stuck stock row
        const uniqueCode = rest.toUpperCase();
        if (orderCodes.has(uniqueCode)) {
          console.warn(`[sheets] CLAIMED:${uniqueCode} row ${i + 1} stuck after delivery — deleting`);
