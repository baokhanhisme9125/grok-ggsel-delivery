...
/**
 * Cleanup stale CLAIMED rows.
 * Handles BOTH marker types:
 *   - CLAIMED:<timestamp>  (old format) → revert if stale > 5 min
 *   - CLAIMED:<uniqueCode> (current)    → if order exists: delete row; else: revert from backup
 */
async function cleanupClaimedRows(sheets, sheetTab, sheetId) {
  try {
    const [stockRes, ordersRes] = await Promise.all([
      ...
      sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: `'${ORDERS_SHEET}'!A:C`,   // A:C to check completed (C has account) vs pending (C empty)
      }).catch(() => ({ data: { values: [] } })),
    ]);

    const rows = stockRes.data.values || [];
    // Only completed orders (column C has account) — pending orders must NOT delete stock rows
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const isTimestampMarker = !isNaN(ts) && ts > 1700000000000;

      if (isTimestampMarker) {
        // Old-style timestamp marker: revert if stale
        if (now - ts > STALE_MS) {
          if (backup) {
            ...
            console.warn(`[sheets] Stale timestamp CLAIMED row ${i + 1} has NO backup`);
          }
        }
      } else {
        // UniqueCode-style marker: CLAIMED:<orderKey>
        // Only delete stock row if order COMPLETED (column C has account).
        // Pending orders must NOT cause stock rows to be deleted.
        ...
        if (completedOrderCodes.has(uniqueCode)) {
          ...
          if (sheetId !== undefined) {
            try {
              ...
              rows.splice(i, 1);
              i--;
            } catch (e) { console.warn(`[sheets] Failed to delete stuck row ${i + 1}:`, e.message); }
          }
        } else {
          // No order found — stuck claim (crash/race) → revert from backup
          if (backup) {
            console.warn(`[sheets] Reverting stuck CLAIMED:${uniqueCode} row ${i + 1} (no order found)`);
            try {
              await sheets.spreadsheets.values.update({
                spreadsheetId: SPREADSHEET_ID,
                range: `'${sheetTab}'!A${i + 1}:B${i + 1}`,
            ...
            console.warn(`[sheets] Stuck CLAIMED:${uniqueCode} row ${i + 1} has NO backup`);
          }
        }
      }
    }
...
async function getNextAvailableAccount(sheetName, uniqueCode) {
  ...
  // Cleanup stale CLAIMED rows in background — only 20% of requests to save API quota
  if (Math.random() < 0.20) {
...
/**
 * Find a COMPLETED order by Digiseller orderId (column E).
 * Prevents double delivery when same order is submitted with different unique codes.
 */
async function findCompletedOrderByOrderId(orderId) {
  ...
  try {
    ...
    const rows = res.data.values || [];
    for (let i = 0; i < rows.length; i++) {
      const rowOrderId = String(rows[i][4] || '').trim();
      const accountCell = (rows[i][2] || '').trim();
      if (rowOrderId === targetId && accountCell.includes(':')) {
        const ci = accountCell.indexOf(':');
        return {
          uniqueCode:      rows[i][0] || '',
          buyerEmail:      rows[i][1] || '',
          accountEmail:    accountCell.slice(0, ci).trim(),
          accountPassword: accountCell.slice(ci + 1).trim(),
          soldAt:          rows[i][3] || '',
          orderId:         rows[i][4] || '',
    ...
    console.warn('[sheets] findCompletedOrderByOrderId failed:', e.message);
  }
  return null;
}
...
module.exports = {
  ...
  isAccountAlreadyDelivered, findRecentOrderByEmail, findCompletedOrderByOrderId, getAllStock,
